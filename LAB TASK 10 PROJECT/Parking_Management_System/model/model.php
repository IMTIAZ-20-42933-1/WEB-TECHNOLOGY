...
function updatePicture($data, $id) {
    $conn = db_conn();
    $selectQuery = "UPDATE drivers set image = ? where id = $id";
    try{
        $stmt = $conn->prepare($selectQuery);
        $stmt->execute([
        	$data['image']
...
{
	$conn = db_conn();
    $selectQuery = "INSERT into `drivers` (name, email, user_name, password, gender, date, image)
VALUES (:name, :email, :user_name, :password, :gender, :date, :image)";
    try{
        $stmt = $conn->prepare($selectQuery);
        $stmt->execute([
        	':name' => $data['name'],
